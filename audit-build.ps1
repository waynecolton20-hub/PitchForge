...
Write-Host 'PITCHFORGE SOURCE AUDIT v4.1.6'
...
$checks = @(
    ...
    [pscustomobject]@{ Pass = ($ps -notmatch 'new\s+\w+|delete\s+|std::malloc|std::free|\.resize\(|\.assign\(' -or $ps -notmatch 'void PitchForgeAudioProcessor::processBlock'); Name = 'No obvious heap-allocation operations are introduced in processBlock' },
    ...
    [pscustomobject]@{ Pass = ($wf -match '(?m)^\s*cmake --build build --config Release --parallel 2\s*$'); Name = 'Windows build command is canonical' },
    [pscustomobject]@{ Pass = ($wf -notmatch 'cmake --build build --config Release --parallel 2 2'); Name = 'Malformed duplicate parallel argument is absent' },
